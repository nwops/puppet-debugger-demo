...
if [ `gem list -i puppet-repl -v '>=0.3.0'` == 'false' ]; then
  sudo gem install puppet-repl --no-rdoc --no-ri # forces puppet install unless already present
fi
...
puppet config set hiera_config "${repl_demo_dir}/hieradata/hiera.yaml" --section main
puppet config set clientcert "foo" --section agent
